...
class Logger {
    private readonly cloudWatchLogger: CloudWatchLogger
    ...
    // Make the log method private to prevent external access
    private async log(entry: LogEntry, level: LogLevel): Promise<void> {
        const logMessage = JSON.stringify({
            level,
            service: this.service,
        ...
        try {
            ...
            console.error(`Failed to log message to CloudWatch: ${error.message}`)
        }
    }
    ...
    /**
     * Middleware for logging HTTP requests and responses.
     * This function is strictly intended for use as Express middleware.
     */
    get logHttp(): (req: Request, res: Response, next: NextFunction) => void {
        return (req: Request, res: Response, next: NextFunction) => {
            const start = Date.now()
